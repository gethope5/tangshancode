...
void PreRunSettingDlg::setBackData()
{
    ...
    qDebug()<<"new rail way pole number"<<ui->wdgPoleNum->getValue();
    ...
    profile.dbpath = m_datapath + "/" + m_dbName;
    ...
    MeasureDB::nPoleNum=ui->wdgPoleNum->getValue().toInt();
    accept();
}
...
void PreRunSettingDlg::setPreRunSetting()
{
    ...
    qDebug()<<"select res: run " <<list;
    if (list.isEmpty())
    {
        QMessageBox::about(this,QTextCodec::codecForName("gbk")->toUnicode("提示"),QTextCodec::codecForName("gbk")->toUnicode("1没有任何铁路路线，请先进基本数据中导入路线！"));
        return;
    }
    m_strLineList = list;
