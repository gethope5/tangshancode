  ...
  #endif
{
    ...
    //    ui->lineRailWidth->setText(QString::number(1234));
    stackRailType =new QStackedLayout;
    newRailPole=new newRailPoleFrm (ui->gpbPoleControl);
    oldRailPole=new oldRailPoleFrm (ui->gpbPoleControl);
    stackRailType->addWidget(newRailPole);
    stackRailType->addWidget(oldRailPole);
    stackRailType->setCurrentIndex(0);
    newRailPole->initialUI(false);
    //forgeCurve();
    ...
    wire2Curves.clear();
    poleCurves.clear();
    wire2Curves<<addWireCurve(Qt::red);

    wire1Curves<<addWireCurve(curWire1Color);
    poleCurves<<addWireCurve(Qt::green);
...
void twoPlotForm::slot_initialPole(bool b,QString initialPole)
{
    ...
    {
        stackRailType->setCurrentIndex(0);
        newRailPole->initialUI(true);
        newRailPole->updateNumber(initialPole);
    }
    else
    {
        stackRailType->setCurrentIndex(1);
        oldRailPole->updateNumber(initialPole);
        oldRailPole->fillPoleSelList(oldRail_idx);
    }
}
...
void twoPlotForm::uiInitial(void)
{
    ...
    if(!m_heightDropperCurve.curveIndex)
    {
        ...
        m_drawWireHeight->graph(m_heightDropperCurve.curveIndex)->setPen(QPen(Qt::red));
    ...
    //承力索,height 7
    heightCarriers<<add1Curve(m_drawWireHeight,Qt::red);
...
void twoPlotForm::drawCarrier(int carrier,double walk)
{
    //    qDebug()<<"carrier"<<carrier<<walk;
    if(carrier)
    {
        curveInfo tmpCarrier=heightCarriers.at(curCarrierIndex);
        bool cc=true;
        if(tmpCarrier.x.count())
        {
            if(tmpCarrier.x.at(tmpCarrier.x.count()-1)==walk)
            {
                cc=false;
        ...
        if(cc)
        {
            tmpCarrier.y<<carrier;
            tmpCarrier.x<<walk;
            heightCarriers.replace(curCarrierIndex,tmpCarrier);
            m_drawWireHeight->graph(tmpCarrier.curveIndex)->setData(tmpCarrier.x,tmpCarrier.y);
...
//        qDebug()<<"wire count+"<<wire1Curves.at(i).first.x.count()<<wire1Curves.at(i).first.y.count();
    ...
    widthPole.clear();
    for(int i=0;i<heightCarriers.count();i++)
    {
        curveInfo  tmp=heightCarriers.at(i);
        tmp.x.clear();
        tmp.y.clear();
        heightCarriers.replace(i,tmp);
        m_drawWireHeight->graph(heightCarriers.at(i).curveIndex)->setData(heightCarriers.at(i).x,heightCarriers.at(i).y);
    }
    curWire2Index=0;
    curWire1Index=0;
    curCarrierIndex=0;
...
void twoPlotForm::slot_dataReplay(QVector<double> dDistance,QVector<QPoint> wire1,QVector<QPoint> wire2,QVector<double> dCarrierHeight,QVector<QPair<double,int> > &poles)
{
    ...
    m_drawWireWidth->replot();
    qDebug()<<"count is"<<dDistance.count();
    //    qDebug()<<"count is"<<  m_drawWireWidth->graph(0)->data()->;//->count();
    m_drawWireHeight->graph(wire1Curves.at(curWire1Index).second.curveIndex)->setData(wire1Curves.at(curWire1Index).second.x,wire1Curves.at(curWire1Index).second.y);
    m_drawWireHeight->graph(wire2Curves.at(curWire2Index).second.curveIndex)->setData(wire2Curves.at(curWire2Index).second.x,wire2Curves.at(curWire2Index).second.y);

    m_drawWireHeight->graph(heightCarriers.at( curCarrierIndex).curveIndex)->setData(heightCarriers.at(curCarrierIndex).x,heightCarriers.at(curCarrierIndex).y);
    m_drawWireHeight->replot();
#else
    for(int i=0;i<wire2Curves.count();i++)
    {
    ...
    m_drawWireWidth->replot();
    m_drawWireHeight->replot();
    qDebug()<<"curve counts,wire1="<<wire1Curves.count()<<"wire2="<<wire2Curves.count()<<"carrier ="<<heightCarriers.count();
#endif
...
void twoPlotForm::processCarrierReplay(QVector<double> &dCarrierHeight,QVector<double> &dDistance)
{
    QVector<int> carrierJudage;
    for(int i=0;i<dCarrierHeight.count();i++)
    {
        if(dCarrierHeight.at(i))
        {
            curveInfo tmp =heightCarriers.at(curCarrierIndex);
            tmp.x<<dDistance.at(i);
            tmp.y<<dCarrierHeight.at(i);
            heightCarriers.replace(curCarrierIndex,tmp);
        ...
        if(carrierJudage.count()==(Another_Curve_STD+1))
        {
            ...
            {
                if(heightCarriers.at(curCarrierIndex).x.count())
                {
                    slot_createCarrier();
                }
            }
...
void twoPlotForm::slot_setDisplayStyle(curveType type,QColor color)
{
    ...
    if(type==curveType::curveColor)//测量数据扫描颜色
    {
    ...
    else if(type==curveType::Dropper)
    {
        ...
        settings.setValue("ScanDiaplayColor/dropper",strColor);

    }
    else if(type==curveType::Carrier)
    {
#if 0
        QPen tmpPen=m_drawWireHeight->graph(m_heightCarrier.curveIndex)->pen();
        tmpPen.setColor(color);
        m_drawWireHeight->graph(m_heightCarrier.curveIndex)->setPen(tmpPen);
#else
        for(int i=0;i<heightCarriers.count();i++)
        {
            QPen tmpPen=m_drawWireHeight->graph(heightCarriers.at(i).curveIndex)->pen();
            tmpPen.setColor(color);
            m_drawWireHeight->graph(heightCarriers.at(i).curveIndex)->setPen(tmpPen);
        }
#endif
        settings.setValue("ScanDiaplayColor/carrier",strColor);
    }
    m_drawWireWidth->replot();
    m_drawWireHeight->replot();
}
...
{
    slot_setDisplayStyle(curveType::curveColor,defaultCurveColor);
    slot_setDisplayStyle(curveType::axisColor,defaultAxisColor);
    slot_setDisplayStyle(curveType::backgroundColor,defaultBackgroundColor);
...
void twoPlotForm::readConfigDisplay(void)
{
    QSettings  settings("Config.ini", QSettings::IniFormat);
    //获取默认配置信息
    defaultCurveColor=settings.value("defaultDiaplayStyle/defaultscanWireColor","red").toString();
    defaultAxisColor=settings.value("defaultDiaplayStyle/defaultaxisLabelColor","white").toString();
    defaultBackgroundColor=settings.value("defaultDiaplayStyle/defaultbackGroundColor","black").toString();
    defaultDrawStyle=settings.value("defaultDiaplayStyle/defaultDrawStyle",false).toBool();
    //获取最近显示配置信息

    strToColor(settings.value("ScanDiaplayColor/scanWireColor","red").toString(),curWire1Color);
    if(curWire1Color.isValid())
    {
        if(wire1Curves.count())
        {
            QPair<curveInfo,curveInfo> tmp=wire1Curves.at(curWire1Index);
            tmp.second.color=curWire1Color;
            tmp.first.color=curWire1Color;
            wire1Curves.replace(curWire1Index,tmp);
        }
    }
    else
    {
        curWire1Color=defaultCurveColor;
        if(wire1Curves.count())
        {
            QPair<curveInfo,curveInfo> tmp=wire1Curves.at(curWire1Index);
            tmp.second.color=defaultCurveColor;
            tmp.first.color=defaultCurveColor;
            wire1Curves.replace(curWire1Index,tmp);
        }
    }

    QColor color;
    strToColor(settings.value("ScanDiaplayColor/axisLabelColor","red").toString(),color);
    if(color.isValid())
    {
        axisColor=color;
    }
    ...
    strToColor(settings.value("ScanDiaplayColor/backGroundColor","red").toString(),color);
    if(color.isValid())
    {
    ...
    strToColor(settings.value("ScanDiaplayColor/dropper","red").toString(),m_heightDropperCurve.color);
    strToColor(settings.value("ScanDiaplayColor/carrier","blue").toString(),m_heightDropperCurve.color);
...
void twoPlotForm::updateMeasureData(MeasureDebugData &databuffer,int nSide,bool flag)
{
    ...
    if(!bNewRailType)
    {
        newRailPole->updateNumber(databuffer.poleID);
    }
...
void twoPlotForm::setCarrierVisible(bool flag)
{
    for(int i=0;i<heightCarriers.count();i++)
    {
        if(heightCarriers.at(i).curveIndex)
        {
            m_drawWireHeight->graph(heightCarriers.at(i).curveIndex)->setVisible(flag);
        }
    }
}
...
{
    heightCarriers<<add1Curve(m_drawWireHeight,Qt::red);
    if(heightCarriers.count())
        curCarrierIndex=heightCarriers.count()-1;
    //    qDebug()<<"created another curve,cur carrier count="<<heightCarriers.count();
}
...
{
    if((e->type()==QEvent::Resize)&&(o==ui->wdgCurveParent))
    {
            updateDrawUI();
    }
    return true;
