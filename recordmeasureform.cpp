...
void recordMeasureForm::on_pbtnOpenFile_clicked()
{
    ...
    else
    {
        ...
        wires1.clear();         //2
        wires2.clear();         //3
        dCarrierHeight.clear(); //4
        poles.clear();
        getFileData(dataFilePaths,wires1,wires2,dCarrierHeight,poles);
        //        qDebug()<<"+++++++++++++++all pole"<<poles;
        emit  dataReplay(dDistance,wires1,wires2,dCarrierHeight,poles);

        openDbUiInitial();
        ui->pbtnDataReplay->setEnabled(true);
    }
}
void recordMeasureForm::getFileData(QString filePath,QVector<QPoint> &wire1,QVector<QPoint> &wire2,QVector<double> &carrierHeight, QVector<QPair<double,int> > &poles)
{
    if(Opendatabase(filePath))
    {
        if(setCurDatabase(&m_db,filePath))
        {
            showDataTable(MeasureDB::detailDataTb);
        }
    }
    if(m_db.isValid())
    {
        ...
        dDistance.clear();
        dCarrierHeight.clear();
        allDist.clear();

        if(query.exec("select id, dist,poleID,wireWidth,wireHeight,wireWidth1,wireHeight1,carrier from debugmeasure"))
        {
            getDataFromDb(query,wire1,wire2,"carrier");
            //            qDebug()<<"new database"<<wire1.count();
            //            qDebug()<<"file data"<<dDistance.count();//<<dWireWidth.count()<<dWireHeight.count();
        }
        else if(query.exec("select id, dist,poleID,wireWidth,wireHeight,wireWidth1,wireHeight1,fXangle from debugmeasure"))
        {
            getDataFromDb(query,wire1,wire2,"fXangle");
            //            qDebug()<<"old database"<<wire1.count();
        }
        else
...
void recordMeasureForm::getDataFromDb(QSqlQuery &query,QVector<QPoint> &wire1,QVector<QPoint> &wire2,QString strCarrier)
{
    ...
    int carrierHeightIndex=query.record().indexOf(strCarrier);
    ...
    while(query.next())
    {
        ...
        dCarrierHeight<<query.value(carrierHeightIndex).toInt();
    }
}
