...
//    ui->groupBox->setVisible(false);
    ...
    ui->pbtnPowerOn->setEnabled(true);
    ui->pbtnUIExpand->setEnabled(true);
    ui->pbtnCurveSyn->setEnabled(true);
    ui->pbtnOverAlarm->setEnabled(true);
    ui->pbtnDeviceConnect->setEnabled(true);
    ui->pbtnLED->setEnabled(true);
    ui->groupBox_2->setVisible(false);
#else
    ui->pbtnPowerOn->setEnabled(false);
    ui->pbtnUIExpand->setEnabled(true);
    ui->pbtnCurveSyn->setEnabled(false);
    ui->pbtnOverAlarm->setEnabled(false);
    ui->pbtnDeviceConnect->setEnabled(false);
    ui->pbtnLED->setEnabled(false);
#endif
}
...
void setForm::updateRadorStatus(bool flag)
{
    ...
    if(flag)
    {
        ...
        if(bFirstPower)
        {
            ...
            {
                //雷达打开后，演示一段时间后，再连接雷达
                qDebug()<<"--------------------------waiting,connect rador";
                QTimer::singleShot(15000,this,SLOT(radorFirstPower()));
                bOpenRadar=false;
            }
            else
            {
...
void setForm::slot_updateIPDeviceStatus(QString buffer)
{
    ...
    if(buffer.contains( "Radar_Open"))
    {
        ...
        setPbtnUI(ui->pbtnPowerOn,true);//显示雷达打开按钮
    }
...
void setForm::radarPowerStack(int n)
{
    if(radorPowerStatus.count()<5)
    {
        radorPowerStatus.push_back(n);
    }
    else
    {
        radorPowerStatus.push_back(n);
        radorPowerStatus.pop_front();
    }
}
