...
//        railComPar( receiveDataBuffer);
        ...
        if(receiveDataBuffer.indexOf("C1:")!=(-1))
        {
            QString tmp=receiveDataBuffer.mid(receiveDataBuffer.indexOf("C1:"),9);
            if(tmp.count()==9)
            {
                parse_RailSensor(tmp,false);
                bOk=true;
            }
        }

        //                qDebug()<<"dist"<<receiveDataBuffer;
        //        receiveDataBuffer.clear();
    }
    //唐山检测车底盘车身倾斜传感器
    else if(nComType==TS_ANGLE_COM)
    {
        if((receiveDataBuffer.indexOf("X")!=(-1))||receiveDataBuffer.indexOf("Y")!=(-1))
        {
            QString tmp=receiveDataBuffer.mid(receiveDataBuffer.indexOf("X"),14);
            if(tmp.count()==14)
            {
                qDebug()<<"angle"<<tmp;
                bOk=true;
...
//            else
//            {
//                qDebug()<<"error angle"<<tmp<<tmp.count();
//            }
        }
//        qDebug()<<"angle"<<receiveDataBuffer;
//        receiveDataBuffer.clear();
    }
    //    receiveDataBuffer.clear();
}
void SerialPort::railComPar(QString & receiveDataBuffer)
{
    ...
    //帧完整性判别
    QString output;//output中存储了一帧完整的数据
    //里程
    if(getParData(receiveDataBuffer,"T",output))
    {
        //            qDebug()<<"-----------------------------walk "<<output;
        parse_RailSensor(output,true);
        output.clear();
        bOk=true;
    }
    //gps error
...
bool SerialPort::connectDevice( )
{
    ...
    else
    {
        ...
        {
            if(m_bOpen)
                debugInfo="tangshan rail com opened";
            else
                debugInfo="tangshan rail com is not opened";
        }
        else if(nComType==TS_ANGLE_COM)
        {
            if(m_bOpen)
                debugInfo="tangshan angle com opened";
            else
                debugInfo="tangshan angle com is not opened";
        }
#endif
    }
    return m_bOpen;
...
void SerialPort:: parse_RailSensor(QString strBuffer, bool flag)
{
    //08-10 上海
    //T+14140000+00471
    //      T-00060016+17853
    ...
    {
        int startIndex=strBuffer.indexOf("C1:");
        curDistanceCount=abs(strBuffer.mid(startIndex+4,5).toInt());
        //qDebug()<<"cur parckage"<<strBuffer<<curDistanceCount;
    }
    if(bFirstWalk)
    {
...
//        qDebug()<<"cur parckage"<<strBuffer<<curDistanceCount<<m_comDataType.walkDistance;
        ...
        {
            m_comDataType.nRailHeight=curRailWidth*sin(m_comDataType.angle*3.1415/180);
            m_comDataType.nRailWidth=curRailWidth/**cos(m_comDataType.angle*3.1415/180)*/;
        }
        //        qDebug()<<"rail"<<strBuffer<<curRailWidth;
#if SYS_COM
...
void SerialPort::cloudControl(CLOUDCONTROL e,int address)
{
    if(!serial->isOpen())
    {
        emit updateShowInfo("cloud serial is not open");
        return ;
    }
    //    qDebug()<<"cloud control";
    //    FF 01 00 08 21 21 4B up
    //    FF 01 00 10 21 21 53 down
    //    FF 01 00 04 21 21 47 left
    //    FF 01 00 02 21 21 45 right
    //    FF 01 00 0A 21 21 4D rightup
    //    FF 01 00 12 21 21 55 rightdown
    //    FF 01 00 0C 21 21 4F leftup
    //    FF 01 00 14 21 21 57 leftdown
    //    FF 01 00 1D 21 21 60 reset
    //    FF 01 00 00 00 00 01 FF 01 00 21 00 00 22 FF 01 00 21 00 00 22
    //0   1       2    3    4     5     6
    //ff,address,cmd1,cmd2,data1,data2,checkSum
    char cmdBuffer[7]={0xff};
    cmdBuffer[1]=address;
    cmdBuffer[2]=0;
    cmdBuffer[4]=0x21;
    cmdBuffer[5]=0x21;
    ...
    else if (e==STOP)
    {
        ...
        char cmdStop[21]={0xFF,0x01,0x00,0x00,0x00,0x00,0x01,0xFF,0x01,0x00,0x21,0x00,0x00,0x22,0xFF,0x01,0x00,0x21,0x00,0x00,0x22};
        cmdStop[1]=address;
        cmdStop[6]=cmdStop[1]+cmdStop[2]+cmdStop[3]+cmdStop[4]+cmdStop[5];
        cmdStop[8]=address;
        cmdStop[13]=cmdStop[9]+cmdStop[10]+cmdStop[1]+cmdStop[12]+cmdStop[8];
        cmdStop[14]=address;
        cmdStop[20]=cmdStop[19]+cmdStop[18]+cmdStop[17]+cmdStop[16]+cmdStop[15];
        serial->write(cmdStop,21);
    }
}
